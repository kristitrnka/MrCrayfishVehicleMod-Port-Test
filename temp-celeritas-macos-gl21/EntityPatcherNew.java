    ...
    public static void patchEntityId(
        ...
        if (parameters.type == ShaderType.GEOMETRY) {
            translationUnit.replaceExpression("entityId",
                    entityInfoComponent("iris_entityInfo[0].x", integerAttributes));

            translationUnit.replaceExpression("blockEntityId",
                    entityInfoComponent("iris_entityInfo[0].y", integerAttributes));

            translationUnit.replaceExpression("currentRenderedItemId",
                    entityInfoComponent("iris_entityInfo[0].z", integerAttributes));
        } else {
            translationUnit.replaceExpression("entityId",
                    entityInfoComponent("iris_entityInfo.x", integerAttributes));

            translationUnit.replaceExpression("blockEntityId",
                    entityInfoComponent("iris_entityInfo.y", integerAttributes));

            translationUnit.replaceExpression("currentRenderedItemId",
                    entityInfoComponent("iris_entityInfo.z", integerAttributes));
        }

        if (parameters.type == ShaderType.VERTEX) {
            // macOS only exposes an OpenGL 2.1 compatibility context. In that
            // context integer generic vertex attributes do not exist, but a
            // floating point constant attribute can carry the small entity IDs
            // exactly and is converted back to int at each use site above.
            translationUnit.injectVariable(
                    flatQualifier + "out " + entityInfoType + " iris_entityInfo;");
            ...
            translationUnit.injectVariable(
                    flatQualifier + "out " + entityInfoType + " iris_entityInfoTCS[];");
            ...
            translationUnit.injectVariable(
                    flatQualifier + "out " + entityInfoType + " iris_entityInfoGS;");
            ...
            translationUnit.injectVariable(
                    flatQualifier + "in " + entityInfoType + " iris_entityInfo;");
